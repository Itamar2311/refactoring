        ...
        for (Performance p : invoice.getPerformances()) {
            ...
            switch (play.getType()) {
                case "tragedy":
                    ...
                    if (p.getAudience() > TRAGEDY_AUDIENCE_THRESHOLD) {
                        thisAmount += TRAGEDY_OVER_BASE_CAPACITY_PER_PERSON * (p.getAudience() - TRAGEDY_AUDIENCE_THRESHOLD);
                    }
                    break;
                case "comedy":
            ...
            result.append(String.format("  %s: %s (%s seats)%n", play.getName(), frmt.format(thisAmount / PERCENT_FACTOR), p.getAudience()));
